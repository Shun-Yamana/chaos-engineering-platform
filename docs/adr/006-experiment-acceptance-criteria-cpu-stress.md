...
| HPA | 対象外 | スポット注入方式採用・HPA 未設定のため自明に満たされる（下記参照） |
...
スポット注入方式（後述）を採用したため HPA は存在しない（HPA: 未設定）。HPA が設定されていない環境で「HPA が不要なスケールアウトをしないこと」を合格基準とすることは自明に満たされ、検証の意味を持たない。

HPA の回復能力を正しく検証するには、実際のリクエスト負荷（ロードジェネレーター）で CPU を上昇させる手法が必要であり、それは別の実験スコープとなる。

### sidecar 注入方式と HPA の相互作用への対処（スポット注入方式採用の根拠）
