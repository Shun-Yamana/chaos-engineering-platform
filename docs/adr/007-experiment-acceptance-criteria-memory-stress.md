...
### 注入方式：スポット注入方式を採用する理由

Deployment spec に sidecar を追加する方式では、ローリングアップデートが発生し全 Pod が stress-ng を持った状態になる。これは「特定 Pod のメモリが圧迫される」という現実の障害とは乖離している。

**当初からスポット注入方式を採用する**（対象 Pod に直接 `stress-ng` を起動）。これにより：
- Deployment spec を変更しないためローリングアップデートが発生しない
- 実験の爆発半径を特定 Pod に限定できる
- 現実の障害（特定 Pod へのメモリ集中）を忠実に再現できる
...
- `memory_stress_inject` の実装はスポット注入方式（Deployment spec 変更なし）で行う
